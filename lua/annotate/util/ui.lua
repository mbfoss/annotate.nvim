...
--- The buffer, if any, named exactly `path`, expanded first. Not `vim.fn.bufnr()`,
--- which matches as a pattern and settles for a partial match.
---@param path string
...
function M.create_window(buffer, enter, config, on_close)
    ...
    local augroup = vim.api.nvim_create_augroup("annotate_window_#" .. win, { clear = true })
...
function M.smart_open_file(filepath, line, col, activate)
    ...
    if not ok or not vim.api.nvim_win_is_valid(winid)
        ...
        if not ok and err and err ~= "" then
            vim.notify("annotate: " .. tostring(err), vim.log.levels.WARN)
        end
        return -1, -1
    end
...
---Pull content down into blank space at the bottom of a window so the viewport
---stays full. Scrolls the view only, and only when earlier lines exist.
---@param winid integer
